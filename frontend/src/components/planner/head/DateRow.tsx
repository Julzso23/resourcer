...
export function DateRow({ dateUnit, dateCells, onScroll, onSetStartDate }: {
  ...
  return (
    <div className="flex flex-row border-b last:border-none border-gray-800 first:rounded-t-lg overflow-hidden relative">
      ...
      { dateCells.map(date => <DateRowCell widthPercent={date[1] * 100} key={date[0]} onClick={() => onSetStartDate(dateUnit, date[0])}>{ getDateString(date[0]) }</DateRowCell>) }

export function TextInput({ type, name, placeholder, autoComplete, value, onChange }: {
  type: string,
  name?: string,
  placeholder?: string,
  autoComplete?: string,
  ...
  return (
    <div className="relative bg-slate-800 rounded-md">
      <input type={type} id={name} name={name} autoComplete={autoComplete} placeholder=" " className={`block px-4 pb-2.5
        ...
        focus:ring-0 focus:border-gray-500 peer rounded-md`} value={value} onChange={onChange} />
      <label htmlFor={name} className={`absolute text-sm text-white/50 duration-100 transform -translate-y-4

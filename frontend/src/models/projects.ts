...
export const projects = createModel<RootModel>()({
  ...
  reducers: {
    ...
    removeProject(state, projectId: number) {
      return { ...state, projects: state.projects.filter(project => project.id !== projectId) }
    },
  ...
  effects: (dispatch) => ({
    ...
    async getProject({ projectId }, rootState) {
      try {
        ...
        this.addProject(await Api.get<ProjectDto>(`projects/${projectId}`, {}, rootState.auth.token || undefined))
      } catch (error) {
        this.handleError({ error })
      }
    },
